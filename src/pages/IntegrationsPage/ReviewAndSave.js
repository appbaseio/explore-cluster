...
import { transformPreferences, transformResultsDefaultFields, removeEmpty } from './utils/index';
...
const ReviewAndSave = ({
	...
	const getDiffData = (oldObj, newObj, isPageLevelDiff = false) => {
		...
		if (!isPageLevelDiff) {
			...
			if (get(diffData, 'pageSettings.pages', null)) {
				...
				const oldVal = get(
					oldObj,
					`pageSettings.pages.${currentPage}.indexSettings.index`,
					'',
				);
				const newVal = get(
					newObj,
			...
			if (get(diffData, 'facetSettings.dynamicFacets', null)) {
				...
				const finalObjArray = [];
				Array.from(new Set([...Object.keys(oldVal), ...Object.keys(newVal)])).forEach(
					(key) => {
	...
	return (
		<div>
			...
			>
				{isOpen && <DiffList diff={[diffData, ...pagesDiffdata]} />}
			</Modal>
		</div>
	);
};
