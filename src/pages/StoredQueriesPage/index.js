...
class StoredQueries extends React.Component {
	...
	handleDelete = (id) => {
		const { deleteStoredQuery, fetchStoredQueries } = this.props;
	...
	constructPayload = () => {
		...
		if (!query) {
			throw new Error('Invalid query, `query` key is missing.');
		}
		if (!formIndexValue) {
			throw new Error('Invalid query, `index` key is missing.');
		}
	...
	handleSaveStoredQuery = () => {
		const { saveStoredQuery, fetchStoredQueries } = this.props;
		...
		try {
			...
			notification.error({
				message: 'There was a problem saving the query.',
			});
	...
	render() {
		...
		if (isLoading && !(storedQueries && storedQueries.length)) {
			return <Loader />;
		}
		return (
			<React.Fragment>
				...
				<Container css={main}>
					<VersionController version="7.48.0">
						<>
							{isDefault && (
								<Card
									style={
								...
								>
									<Table
										...
										dataSource={storedQueries.map((item) => ({
