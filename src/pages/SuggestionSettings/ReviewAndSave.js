...
import { diff } from 'jsondiffpatch'; // eslint-disable-next-line import/no-cycle
...
const ReviewAndSave = ({ oldData, newData, isLoading }) => {
	...
	const getDiffData = (oldObj, newObj) => {
		...
		if (get(diffData, 'popularSuggestions.externalSuggestions', null)) {
			const newVal = get(newObj, 'popularSuggestions.externalSuggestions', []);
			const oldVal = get(oldObj, 'popularSuggestions.externalSuggestions', []);

			diffData = {
				...diffData,
				popularSuggestions: {
					...diffData.popularSuggestions,
					externalSuggestions: [oldVal, newVal],
				},
			};
		}
